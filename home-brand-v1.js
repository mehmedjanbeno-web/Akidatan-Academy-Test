(()=>{
  ...
  renderHome=function(){
    ...
    /* final14: authoritative phone home geometry via inline !important */
    ...
    imp(hero,'height','168px');imp(hero,'min-height','168px');imp(hero,'max-height','168px');imp(hero,'box-sizing','border-box');
    ...
    if(grid){
      ...
      imp(grid,'grid-template-rows','76px 76px');imp(grid,'grid-auto-rows','76px'); imp(grid,'height','158px');imp(grid,'gap','6px');imp(grid,'margin','0');
      [...grid.children].forEach(card=>{
        imp(card,'position','static');imp(card,'transform','none');imp(card,'top','auto');
        imp(card,'height','76px');imp(card,'min-height','76px');imp(card,'max-height','76px');
        imp(card,'padding','6px 9px');imp(card,'box-sizing','border-box');imp(card,'overflow','hidden');
        const ti=card.querySelector('.quick-title'),sub=card.querySelector('.quick-sub'),ico=card.querySelector('.quick-icon');
        imp(ti,'font-size','12px');imp(ti,'line-height','1.05');
        imp(sub,'font-size','8px');imp(sub,'line-height','1.05');
        imp(ico,'width','30px');imp(ico,'height','30px');imp(ico,'flex-basis','30px');
        if(state.theme==='dark'){imp(ti,'color','#f4f8fc');imp(ti,'-webkit-text-fill-color','#f4f8fc');imp(sub,'color','#a9bed0');imp(sub,'-webkit-text-fill-color','#a9bed0')}else{imp(ti,'color','#17324d');imp(ti,'-webkit-text-fill-color','#17324d');imp(sub,'color','#60778d');imp(sub,'-webkit-text-fill-color','#60778d')}
      });
    }
    imp(progressHead,'position','static');imp(progressHead,'height','18px');imp(progressHead,'min-height','18px');imp(progressHead,'margin','1px 2px 0');
    ...
    imp(progress,'position','static');imp(progress,'height','34px');imp(progress,'min-height','34px');imp(progress,'max-height','34px');imp(progress,'padding','2px 8px');imp(progress,'margin','0');imp(progress,'overflow','hidden');
